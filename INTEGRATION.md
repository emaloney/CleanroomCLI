...
*Integration* is the act of embedding the `CleanroomCLI.framework` binary into your project, thereby exposing the API it provides to your code.

CleanroomCLI is built as a *Swift framework*, and as such, it has the following base platform requirements:
...
Apple TV|tvOS |tvOS 
Apple Watch|watchOS |watchOS 

**This is the `universal-framework` branch of CleanroomCLI.** It uses **Swift ** and **requires Xcode ** to compile.
...
The steps below have been tested with **git **, although they should be compatible with a wide range of recent git versions.
...
- **[Carthage integration](#carthage-integration)** — Explains how to use the [Carthage](https://github.com/Carthage/Carthage) dependency manager for adding CleanroomCLI to your project.

- **[Manual integration](#manual-integration)** — Demonstrates the steps for adding CleanroomCLI by embedding `CleanroomCLI.xcodeproj` within your own Xcode project. (**Note:** Manual integration is a bit more involved than using Carthage).
...
> As of this writing, the current version of Carthage is .
...
When building a framework for a platform with a simulator, Carthage creates a *universal binary*, allowing `CleanroomCLI.framework` to work in the simulator as well as on actual devices.
...
1. Update the `Cartfile` with an entry for CleanroomCLI
2. Download and build CleanroomCLI
3. Add `CleanroomCLI.framework` to your application target
...
```
github "emaloney/CleanroomCLI"
```

### 2. Download & Build using Carthage

The command `carthage update` causes Carthage to download and build CleanroomCLI.

By default, `carthage update` builds all platform targets in a project. Normally, this is _not_ what you want; you’ll usually use CleanroomCLI on just one platform at a time.
...
Once Carthage is done building CleanroomCLI, you can use the `open` command to locate the framework binaries in Finder:
...
If all went well, the Carthage build directories should contain the file `CleanroomCLI.framework`. If that file isn’t present, something went wrong with the build.
...
```
"$PROJECT_DIR"/Carthage/Checkouts/CleanroomCLI/BuildControl/bin/stripCarthageFrameworks.sh
```
...
Once you’ve done this, try building your application. If you don’t see any errors, **_you’re all done integrating CleanroomCLI!_**

But before you start coding, skip to the [Adding the Swift import](#adding-the-swift-import) section to see how you can import CleanroomCLI for use in your Swift code.

## Manual Integration

Manual integration involves embedding `CleanroomCLI.xcodeproj` directly in your Xcode project.

This will ensure that CleanroomCLI is built with the exact same settings you’re using for your app. You won’t have to fiddle with different settings for different architectures — when you’re running in the simulator, it will work; when you then switch to building for device, it’ll work, too.

You’ll also be able to step into CleanroomCLI code directly in the debugger without worrying about `.dSYM` resolution, which is very helpful.

### An Overview of the Process

Manual integration is a bit involved; there are five high-level tasks that you’ll need to perform:

1. Download the CleanroomCLI source into your project structure
2. Embed `CleanroomCLI.xcodeproj` in your Xcode project
3. Build `CleanroomCLI.framework`
4. Add `CleanroomCLI.framework` to your application target
...
For our integration examples, we’re going to be showing the top-level `CleanroomCLI` directory inside a `Libraries` directory at the root level of of your application’s source.
...
### 1. Download the CleanroomCLI source

If you’re already using git for version control, we recommend adding CleanroomCLI to your project as a submodule. This will allow you to “lock” your codebase to specific versions of CleanroomCLI, making it easier to incorporate new versions on whatever schedule works best for you.

If you’re using some other form of version control of if you’re not using version control at all—*shame on you!*—then you’ll want to *clone* the CleanroomCLI repository. We suggest putting the CleanroomCLI clone somewhere within your application’s directory structure, so that it is included in whatever version control regimen you’re using.

#### Downloading CleanroomCLI as a submodule

> **Important:** Skip this section if you plan to download CleanroomCLI using `git clone`.

From within the `Libraries` directory, issue the following commands to download CleanroomCLI:

```bash
git submodule add https://github.com/emaloney/CleanroomCLI
git submodule update --init --recursive
```

Next, you’re ready to [embed the `CleanroomCLI.xcodeproj` project file in your Xcode project](#2-embed-cleanroomcli-in-your-project).

#### Downloading CleanroomCLI as a cloned repo

> **Important:** Skip this section if you already performed the tasks outlined in “Downloading CleanroomCLI as a submodule” above.

From within the `Libraries` directory, issue the following command to clone the CleanroomCLI repository:

```bash
git clone https://github.com/emaloney/CleanroomCLI
```

### 2. Embed CleanroomCLI in your project

In the Terminal, the command `open CleanroomCLI` to open the folder containing the CleanroomCLI source in the Finder. This will reveal the `CleanroomCLI.xcodeproj` Xcode project and all files needed to build `CleanroomCLI.framework`.

Then, open your application in Xcode, and drag `CleanroomCLI.xcodeproj` into the Xcode project browser. This will embed CleanroomCLI in your project and allow you to add the targets built by CleanroomCLI to your project.

### 3. Build CleanroomCLI

Before we can add `CleanroomCLI.framework` to your app, we have to build it, so Xcode has more information about the framework.
...
Once a device-based run destination has been selected, select the appropriate build scheme for the target platform: “CleanroomCLI-iOS”, “CleanroomCLI-macOS”, “CleanroomCLI-tvOS” or “CleanroomCLI-watchOS”.

Then, select *Build* (⌘B) from the *Product* menu.

Once the build is complete, open `CleanroomCLI.xcodeproj` in the project navigator and find the “Products” group. Open that, and right-click on `CleanroomCLI.framework`. Select *Show in Finder*. This will open the folder containing the framework binary you just built.

You may see several files in this folder; the one we’re concerned with is:

- `CleanroomCLI.framework`
...
So, to make things sane again, you’ll need to make sure Xcode references `CleanroomCLI.framework` using a “Relative to Build Products” location.

To do this:

1. Locate `CleanroomCLI.framework` in the Xcode project browser
...
Once you’ve done this for each framework, **_you’re all done integrating CleanroomCLI!_**

## Adding the Swift import

Once CleanroomCLI has been successfully integrated, all you will need to do is add the following `import` statement to any Swift source file where you want to use CleanroomCLI:
